...
class ShortestPath:
    ...
    def _update_path(self, path: np.ndarray, v) -> None:
        for i in range(len(path) - 1):
            self.graph_[path[i]][path[i + 1]] = self.graph_[path[i]][path[i + 1]] * (1 + 2 * (v / 40) ** 2)
    ...
    def fit_flow(self, graph_: dict, pos: dict, start: int, target: int, flow: int, step: int) -> None:
        """
        Fits the given graph with the A* algorithm to find the shortest path from a start node to a target node.
        ...
        self.start = start
        self.target = target
        for i in range(0, flow, step):
            self.graph_data = {start: 0}
            self.shortest, self.path = self._astar_method()
            self.shortest_path = self.shortest[target]
            self.graph_plotting(self.path, pos=pos, name=f"Question2_step{i}")
    ...
    def graph_plotting(self, given_path: np.ndarray, pos: dict, name: str = "Question2_tree.svg"):
        """
