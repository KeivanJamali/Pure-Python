...
actor_prompt_template = ChatPromptTemplate([
    ("system", """You are an expert researcher.
     ...
     partial_variables={"time": lambda: datetime.datetime.now().isoformat()},
     input_variables=["first_instruction", "messages"],
     validate_template=True,
     metadata={"name": "actor_prompt"})

first_responder_prompt_template = actor_prompt_template.partial(first_instruction="Provide a detailed ~250 words answer.")
...
- Add a "Refrence" section to the bottom of your answer (which does not count towards the word limit). in form of:
    - [1] https://example.com
    - [2] https://example.com
