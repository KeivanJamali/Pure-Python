...
# Absolute paths
RAW_DATA_DIR = r'D:\All Python\Pure-Python\P2\12-Work-GITI\04-ThirdCode-Generic\Webapp\raw-data'
RESULT_DIR = r'D:\All Python\Pure-Python\P2\12-Work-GITI\04-ThirdCode-Generic\Webapp\result'
...
def process_file(file, epsilon, roun_lim):
    ...
    csv_file, txt_file, out_ranges_file = dataloader.save_file(RESULT_DIR)
...
def index():
    ...
    if request.method == 'POST':
        ...
        if input_file and epsilon and roun_lim:
            ...
            os.makedirs(os.path.dirname(input_filepath), exist_ok=True)  # Ensure upload directory exists
            input_file.save(input_filepath)
            
            csv_file, txt_file, out_ranges_file = process_file(input_filepath, epsilon, roun_lim)
            session['files'] = [os.path.basename(csv_file), os.path.basename(txt_file), os.path.basename(out_ranges_file)]
