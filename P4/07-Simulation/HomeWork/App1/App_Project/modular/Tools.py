class Queue:
    ...
    def depart(self, id:str, time:float):
        ...
        temp = self.time_in_queue[-1][1] if self.time_in_queue else 0
        self.time_in_queue.append([id, temp + (self.history[id][1] - self.history[id][0])])
        # If we want to remove useless data use the folloing line.
        # self.history.pop(id, None)
        self.total_time_in_queue = self.time_in_queue[-1][1]
...
class Server:
    ...
    def occupied_at(self, id:str, time:float):
        ...
        self.last_id = id
        self.occupied = True
        self.total_time = self.occupied_time + self.unoccupied_time
    ...
    def check_correction(self, time):
        use = False
        for key, value in self.history.items():
            if value[0] > time:
                use = True
                self.history[key] = [value[0], value[0]]
                self.occupied_time = 0
        if use:
            for key, value in self.history.items():
                self.occupied_time += value[1] - value[0]
