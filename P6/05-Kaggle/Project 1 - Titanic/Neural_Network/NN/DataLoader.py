...
class MyDataloader:
    def __init__(self, file_path: str, train_percent: float, val_percent: float, test_percent: float, batch_size: int, random_seed: int=42):
        """
        ...
        self.data = self._setting(self.raw_data.copy())
    ...
    def _setting(self, data:pd.DataFrame) -> pd.DataFrame:
        """
        ...
        missed_number = data["Age"].isna().sum()
        ...
        embarked_values = data["Embarked"].dropna()
        missed_number = data["Embarked"].isna().sum()
        imputed_values = np.random.choice(embarked_values, size=missed_number, replace=True)
        data.loc[data["Embarked"].isna(), "Embarked"] = imputed_values
        data["Embarked"] = data["Embarked"].map({"S":0, "C":1, "Q":2})

        data = data.loc[:, info.target+info.features]
        data = pd.get_dummies(data, columns=["Embarked", "Survived"], dtype=int)
        data = data.astype(float)
        info.features_dataloader = data.columns[:-2]
        info.target_dataloader = data.columns[-2:]
        return data
    ...
    def feature_importance_data(self, feature:str=None, seed=42, data_choice:str="train"):
        result_data = {}
        if data_choice == "train":
            raw_true_data, second_data = train_test_split(self.train_data.copy(), test_size=0.5, random_state=seed)
        elif data_choice == "test":
            raw_true_data, second_data = train_test_split(self.test_data.copy(), test_size=0.5, random_state=seed)
