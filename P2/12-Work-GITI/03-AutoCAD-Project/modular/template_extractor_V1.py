...
def generate_entity_code(entity):
    try:
        if entity.EntityName == 'AcDbLine':
            ...
            return f"acad.model.AddLine(APoint({start[0]}, {start[1]}, {start[2]}), APoint({end[0]}, {end[1]}, {end[2]}))"
        elif entity.EntityName == 'AcDbCircle':
            center = entity.Center
            radius = entity.Radius
            return f"acad.model.AddCircle(APoint({center[0]}, {center[1]}, {center[2]}), {radius})"
        elif entity.EntityName == 'AcDbText':
            insert = entity.InsertionPoint
            text = entity.TextString
            return f"acad.model.AddText('{text}', APoint({insert[0]}, {insert[1]}, {insert[2]}), {entity.Height})"
        elif entity.EntityName == 'AcDbArc':
            center = entity.Center
            radius = entity.Radius
            start_angle = entity.StartAngle
            end_angle = entity.EndAngle
            return f"acad.model.AddArc(APoint({center[0]}, {center[1]}, {center[2]}), {radius}, {start_angle}, {end_angle})"
        else:
