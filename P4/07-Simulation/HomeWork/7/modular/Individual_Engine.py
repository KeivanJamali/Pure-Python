...
class Queue:
    ...
    def _dispatcher_process(self, policy_function, detailed:bool=False):
        ...
        """
        while True:
            dispatcher_event = self._callbacks["dispatcher_event"]
            arrive_event = self._callbacks["customer_arrived_event"]

            result_event = yield arrive_event | dispatcher_event
            if arrive_event in result_event:
                if len(self.server.users) < self.server.capacity and len(self.customers_in_queue) > 0:
                    ...
                    if detailed:
                        print(f"[FREE SPOT] Customer {customer_id} goes to server because it has more space.")

                elif len(self.customers_in_queue) == 0 and len(self.server.users) == 0:
                    customer_id = yield arrive_event
                    customer_data = self.customer_in_system[customer_id]
                    del self.customer_in_system[customer_id]
                    for key, value in self.customer_in_system.items():
                        self.leave_interarrival.append(value.leave_time)
                    self.customer_in_system = {customer_id:customer_data}
                    self.customer_in_system[customer_id].active()
            ...
            elif dispatcher_event in result_event:
                customer_id = yield dispatcher_event
                self.leave_interarrival.append(self.customer_in_system[customer_id].leave_time)
                del self.customer_in_system[customer_id]
                if detailed:
    ...
    def run(self, policy, report:bool=True, detailed:bool=False) -> None:
        ...
        leave_interarrival2 = []
        for i in range(len(self.leave_interarrival)-1):
            leave_interarrival2.append(self.leave_interarrival[i+1] - self.leave_interarrival[i])
        self.leave_interarrival = leave_interarrival2
    ...
    def output_generator(self):
        class Deterministic_Generator:
            """Generating a constant number in a list, only by calling the object."""
            ...
            def __call__(self):
                if len(self.items):
                    ...
                    return float("inf")
        gen = Deterministic_Generator(items=self.leave_interarrival)
        return gen
