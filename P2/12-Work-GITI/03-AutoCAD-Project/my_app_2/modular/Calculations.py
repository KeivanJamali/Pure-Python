...
class Calculate:
    def __init__(self, file, progress_callback=None):
        ...
        if self.progress_callback:
            self.progress_callback(0.60)
        self._store_tables()
        if self.progress_callback:
            self.progress_callback(0.85)
        self.data = {}
        ...
        if not self.massage1:
            ...
            self.massage2, self.massage3 = None, None
        if self.progress_callback:
    ...
    def _config(self, file):
        ...
        self.city_top = data.iloc[16, 1]
        self.city_down = data.iloc[17, 1]

        if data.iloc[18, 1] == True or data.iloc[18, 1] == "TRUE":
            self.left = "UP"
            self.right = "DOWN"
        elif data.iloc[18, 1] == False or data.iloc[18, 1] == "FALSE":
            self.left = "DOWN"
            self.right = "UP"
        else:
            self.left = ""
        ...
        for H in [h_min, h_max_l, h_max_r]:
            if not t3[t3["H"]==H].empty:
                ...
                if H > 7 or H < 1:
                    ...
                    return f"[ERROR] Max H is incorrect. H = {H}. should be between 1 and 7."
    ...
    def apply_table1(self, d: pd.Series) -> None:
        ...
        print(f"[INFO] We found the H={self.H}. Now let's check with {d["H"]}.")
        if self.H >= d["H"]:
            print(f"[Wrong]...")
            self.H = d["H"]
    ...
    def apply_table3(self, data_list:list):
        ...
        w1_code_first_pei = round(self.data["i-101"] - data_list[2]["m"], 2)
        w2_code_first_pei = round(self.data["i-101"] - data_list[2]["m"], 2)
        w3_code_first_pei = round(self.data["i-104"] - data_list[1]["m"], 2)
        w4_code_first_pei = round(self.data["i-104"] - data_list[1]["m"], 2)

        if self.data["i-092"] == "UP":
            w1_code_end_pei = round(self.data["i-101"] + self.z_nature * w1_distance - data_list[0]["m"], 2)
            w2_code_end_pei = round(self.data["i-101"] + self.z_nature * w2_distance - data_list[0]["m"], 2)
            w3_code_end_pei = round(self.data["i-104"] - self.z_nature * w3_distance - data_list[0]["m"], 2)
            w4_code_end_pei = round(self.data["i-104"] - self.z_nature * w4_distance - data_list[0]["m"], 2)
        elif self.data["i-092"] == "DOWN":
            w1_code_end_pei = round(self.data["i-101"] - self.z_nature * w1_distance - data_list[0]["m"], 2)
            w2_code_end_pei = round(self.data["i-101"] - self.z_nature * w2_distance - data_list[0]["m"], 2)
            w3_code_end_pei = round(self.data["i-104"] + self.z_nature * w3_distance - data_list[0]["m"], 2)
        ...
        self.data["i-136"] = w3_code_first_pei
        self.data["i-150"] = w4_code_first_pei

        self.data["i-110"] = w1_code_end_pei
        self.data["i-124"] = w2_code_end_pei
        self.data["i-137"] = w3_code_end_pei
        self.data["i-151"] = w4_code_end_pei

        self.data["i-111"] = code_upper_pei
        self.data["i-125"] = code_upper_pei
        self.data["i-138"] = code_upper_pei
        self.data["i-152"] = code_upper_pei
