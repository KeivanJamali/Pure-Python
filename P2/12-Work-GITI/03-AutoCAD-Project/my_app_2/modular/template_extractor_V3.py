...
class Extract_template():
    def __init__(self, file_name, time_load=30, progress_callback=None):
        ...
        print("[AutoCad oppened properly.]")
    ...
    def generate(self, output_file_dir:str, output_file_dwg, want_time_line:bool=False) -> None:
        ...
        for attempt in range(max_retries):
            try:
                if want_time_line:
                    try:
                        # Attempt to list entities
                        entities = list(self.acad.iter_objects())
                    except Exception as e:
                        print(f"Error iterating objects: {e}")
                        # Check if the method exists
                        if not hasattr(self.acad, 'iter_objects'):
                            print("Method 'iter_objects' is not available in this AutoCAD version.")
                        continue
