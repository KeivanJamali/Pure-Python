...
class Engine:
    ...
    def fit(self, alpha, if_plot_tree=False):
        self._set_setting()
        self._split_data()
        self._decision_tree(if_plot_tree=if_plot_tree, alpha=alpha)

    def _set_setting(self):
        self.y = self.data[info.target]
        self.x = self.data.drop(info.target, axis=1)
        print(f"[INFO] Data splitted to X and Y.")

    def _split_data(self):
        self.x_train, self.x_test, self.y_train, self.y_test = train_test_split(self.x, self.y, test_size=0.3, random_state=self.seed)

    def _decision_tree(self, if_plot_tree, alpha):
        tree = DecisionTreeRegressor(random_state=self.seed, ccp_alpha=alpha)
    ...
    def find_best_alpha(self):
        ...
        trees = []
        for i in tqdm(range(len(ccp_alphas))):
            dt = DecisionTreeRegressor(random_state=42, ccp_alpha=ccp_alphas[i])
            dt.fit(self.x_train, self.y_train)
            trees.append(dt)

        # Step 3: Evaluate each tree
        train_scores = [tree.score(self.x_train, self.y_train) for tree in trees]
        test_scores = [tree.score(self.x_test, self.y_test) for tree in trees]

        best_index = np.argmax(test_scores)
        best_alpha = ccp_alphas[best_index]
        print(f"best alpha for test is:{best_alpha}")
    ...
    def cross_validation_to_find_best_alpha(self):
        ...
        for i in tqdm(range(len(ccp_alphas))):
            tree = DecisionTreeRegressor(random_state=self.seed, ccp_alpha=ccp_alphas[i])
            scores = cross_val_score(tree, self.x_train, self.y_train, cv=5)
            alpha_loop_values.append([ccp_alphas[i], np.mean(scores), np.std(scores)])

        self.alpha_result = pd.DataFrame(alpha_loop_values,
                                    columns=["alpha", "mean_accuracy", "std"])

        self.alpha_result.plot(x="alpha",
                          y="mean_accuracy",
                          yerr="std",
                          marker="o",
                          linestyle="--")
    ...
    def individual_conditional_expectation(self, features:dict):
        for feature, values in features.items():
            ...
            for i in new_data.index:
                ...
                plt.plot(values, y_new_data_i, c="blue", alpha=0.01)
