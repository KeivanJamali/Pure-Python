...
class Queue_Simulator:
    ...
    def _log(self, last:bool=False, detailed:bool=False) -> None:
        ...
        N = N_Q + U
        people_45 = 100*self.system.count_45_min_id/self.number_of_ids
        print("=======================================================")
        if not last:
        ...
        if detailed:
            ...
            if self.policy == "FCFS":
                ...
                print(f"Total time in queue is {self.queue.time_in_queue} -> {self.queue.time_in_queue[self.id_departure]}") if self.departures else 0
        print()
        print(f"Therefore the U={U}")
        print(f"We have {self.system.count_45_min_id} people with more that 4.5 min system time. Therefore the percentage is {people_45}")
        print(f"Therefore, Average Number of People in Queue (N_Q) will be {N_Q}")
        print(f"Therefore, Average waiting time for People in Queue (Q_bar) will be {Q_bar}")
