...
class Engine:
    def __init__(self, model: torch.nn.Module,
                 ...
                 pretrained_dataloader: torch.utils.data.DataLoader = None,
                 pre_model: torch.nn.Module = None,
                 learned_distribution: Any = None,
                 random_seed: int=42):
            """
            ...
            self.pretrained_dataloader = pretrained_dataloader
            self.distribution = learned_distribution
            self.device = None
            self.result = None
            self.train_true_predict_list, self.val_true_predict_list, self.test_true_predict_list = None, None, None

    def _train_step(self, loss_fn, optimizer_pre, optimizer_main):
        self.pre_model.train()
        self.model.train()
        train_loss, train_acc = 0, 0
        true_predict_list = {"true":[], "predict":[]}

        for batch, (x1, x2, x, y) in enumerate(self.train_dataloader):  
            x1, x2 = x1.to(self.device), x2.to(self.device)
            x, y = x.to(self.device), y.to(self.device).float()

            # === Forward pass ===
            z = self.pre_model(x1, x2)          # intermediate representation
            y_logit = self.model(z, x)            # pass both x and z to fine-tune model
            y_logit = y_logit.squeeze()
            # print("z is", z[0:3])
            # print("prediction", y_logit[0:5])
            # print("true", y[0:5])
            ...
            y_pred_labels = torch.round(y_logit)
            train_acc += (y_pred_labels == y.float()).sum().item() / len(y)
    ...
    def _val_step(self, loss_fn):
        self.pre_model.eval()
        self.model.eval()
        val_loss, val_acc = 0, 0
        true_predict_list = {"true":[], "predict":[]}

        for batch, (x1, x2, x, y) in enumerate(self.val_dataloader):
            x1, x2 = x1.to(self.device), x2.to(self.device)
            x, y = x.to(self.device), y.to(self.device).float()
            ...
            y_pred_labels = torch.round(y_logit, decimals=0)
            val_acc += (y_pred_labels == y.float()).sum().item() / len(y)
    ...
    def train(self,
              ...
              optimizer_pre: torch.optim.Optimizer,
        ...
        for epoch in tqdm(range(1, epochs_num + 1)):
            train_loss, train_acc, true_predict_list = self._train_step(loss_fn=loss_fn, optimizer_pre=optimizer_pre, optimizer_main=optimizer_main)
