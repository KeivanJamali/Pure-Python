...
class Eco:
    ...
    def print_factor(self, factor: float, name: str, is_parametric: bool=False) -> None:
    ...
    def check_if_parametric(self, parameter_list: list) -> bool:
...
class Primary_Eco(Eco):
    """Updated for parametric solutions"""
    def __init__(self, decimal_digits: int = 4):
        super().__init__()
        self.decimal_digits = decimal_digits

    def p_to_f(self, i: float, n: int, p: float = None, print_: bool = False) -> float:
        """produce f from p"""
        factor = (1 + i) ** n
        if print_:
            self.print_factor(factor=factor, 
                              name=f"(F/P, {i}, {n})", 
                              is_parametric=self.check_if_parametric([i, n, p]))
        if p:
            return p * factor
        else:
            return factor

    def f_to_p(self, i: float, n: int, f: float = None, print_: bool = False) -> float:
        """produce p from f"""
        factor = 1 / (1 + i) ** n
        if print_:
            self.print_factor(factor=factor, 
                              name=f"(P/F, {i}, {n})",
                              is_parametric=self.check_if_parametric([i, n, f]))
        if f:
            return f * factor
        else:
            return factor

    def a_to_p(self, i: float, n: int = None, a: float = None, print_: bool = False) -> float:
        """produce p from a"""
        if n:
            ...
            if print_:
                self.print_factor(factor=factor, 
                                  name=f"(P/A, {i}, {n})",
                                  is_parametric=self.check_if_parametric([i, n, a]))
            if a:
                return a * factor
            else:
                return factor
            ...
            if print_:
                self.print_factor(factor=factor, 
                                  name=f"(P/A for inf years, {i})",
                                  is_parametric=self.check_if_parametric([i, a]))
            if a:
                return a * factor
            else:
                return factor

    def p_to_a(self, i: float, n: int, p: float = None, print_: bool = False) -> float:
        """produce a from p"""
    ...
    def a_to_f(self, i: float, n: int, a: float = None, print_: bool = False) -> float:
        """produce f from a"""
        factor = ((1 + i) ** n - 1) / i
        if print_:
            self.print_factor(factor=factor, 
    ...
    def f_to_a(self, i: float, n: int, f: float = None, print_: bool = False) -> float:
        """produce a from f"""
        factor = i / ((1 + i) ** n - 1)
        if print_:
            self.print_factor(factor=factor, 
    ...
    def g_to_p(self, i: float, n: int, g: float = None, print_: bool = False) -> float:
        """produce p from g"""
        factor = (1 / i) * ((((1 + i) ** n - 1) / (i * (1 + i) ** n)) - (n / (1 + i) ** n))
        if print_:
            self.print_factor(factor=factor, 
                              name=f"(P/G, {i}, {n})",
                              is_parametric=self.check_if_parametric([i, n, g]))
        if g:
            return g * factor
        else:
            return factor

    def g_to_a(self, i: float, n: int, g: float = None, print_: bool = False) -> float:
        """produce a from g"""
    ...
    def a1_to_p(self, i: float, j: float, n: int, a1: float = None, print_: bool = False) -> float:
        """produce p from a1"""
        factor1 = (n * a1) / (1 + i)
            ...
            if i == j:
                if print_:
                    self.print_factor(factor=factor1,
                                      name=f"(P/A1, {i}, {j}, {n})",
                                      is_parametric=self.check_if_parametric([i, j, n, a1]))
                return a1 * factor1
            else:
                if print_:
    ...
    def a1_to_f(self, i: float, j: float, n: int, a1: float = None, print_: bool = False) -> float:
        """produce f from a1"""
        factor1 = n * (1 + j) ** (n - 1)
    ...
    def rg(self, parameters: list, print_: bool = False) -> float:
    ...
    def ie(self, r: float, m: int, print_: bool = False) -> float:
        """produce effective interest rate"""
        factor = (1 + r / m) ** m - 1
        if print_:
            self.print_factor(factor=factor, 
                              name=f"(ie, r={r}, m={m})",
                              is_parametric=self.check_if_parametric([r, m]))
        return factor

    def i_from_ie(self, ie: float, m: int, print_: bool = False) -> float:
        """produce interest rate from effective interest rate"""
        factor = (ie + 1) ** (1 / m) - 1
        if print_:
            self.print_factor(factor=factor, 
                              name=f"(i_from_ie, ie={ie}, m={m})",
                              is_parametric=self.check_if_parametric([ie, m]))
        return factor

    def some_f_to_p(self, i: float, all_f: list, with_p: bool = False) -> float:
